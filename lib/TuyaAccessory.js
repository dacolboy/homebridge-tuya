...
class TuyaAccessory extends EventEmitter {
    constructor(props) {
        ...
        if (props.connect !== false) this.connect();
    ...
    connect() {
        ...
        (this._socket.reconnect = () => {
            this.log.info(`reconnect called for ${this.context.name}`);
    ...
    disconnect() {
        if (this._socket) {
            this.log.info('desconectando %s', this.context.name);
            this._socket.destroy();
            if (this._socket._pinger) {
                clearTimeout(this._socket._pinger);
                this._socket._pinger = null;
    ...
    _msgHandler_3_3(task, callback) {
        ...
        switch (cmd) {
            ...
            case 10:
                if (data) {
                    if (data.dps) {
                        this.log.info(`Heard back from ${this.context.name} with command ${cmd}:`, JSON.stringify(data.dps));
                        this._change(data.dps);
                    } else {
            ...
            case 16:
                if (parsedPayload) {
                    if (parsedPayload.dps) {
                        this.log.info(`Heard back from ${this.context.name} with command ${cmd}:`, JSON.stringify(parsedPayload.dps));
