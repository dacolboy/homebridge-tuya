class BaseAccessory {
    constructor(...props) {
        ...
        this.device.once('connect', () => {
            ...
            self.accessory.services.forEach(service => {
        ...
        this.device.once('change', () => {
            this.device.state = {...this.device.state, ...this.device.context.initState};
    ...
    getState(dp, callback) {
        if (!this.device.connected) return callback(new this.hap.HapStatusError(this.hap.HAPStatus.SERVICE_COMMUNICATION_FAILURE));
        const _callback = () => {
            if (Array.isArray(dp)) {
                const ret = {};
                dp.forEach(p => {
